/**
@file arducam_ov5642_loop.cpp

@brief Modified test program for Raspberry Pi 4B. This version will work
only on the 5642 camera chip module and capture continuously an image
for testing until the program is not stopped.
...
int main(int argc, char *argv[])
{
  ...
  // Loop forever capturing images until the program is not stopped
  while(true) {
       // Flush the FIFO
      myCAM.flush_fifo();    
      ...
      // Transfer the camera FIFO buffer and ave the image on file in abour 8 sec.
      // at the max resolution *5 Mp)
