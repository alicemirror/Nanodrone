...
Removed the unwanted serial messages. E.M.
...
// Undef to remove serial output reporting
#undef _NO_ERRORS_REPORT
...
#ifdef _NO_ERRORS_REPORT
  TF_LITE_REPORT_ERROR(error_reporter, "Reading %d bytes from Arducam", jpeg_length);
#endif
...
TfLiteStatus DecodeAndProcessImage(tflite::ErrorReporter* error_reporter,
                                   ...
                                   uint8_t* image_data) {
#ifdef _NO_ERRORS_REPORT
  TF_LITE_REPORT_ERROR(error_reporter, "Decoding JPEG and converting to greyscale");
#endif
